...
public class HelixFrame : MonoBehaviour
{
	...
	void Start()
    {
        ...
        coordinates = new Vector3[39];
        
        for (int i = 0; i<= 36; i+=3)
        {
            coordinates[i] = new Vector3(0,i*height*2.5f,0);
            coordinates[i+1] = new Vector3((distance * Mathf.Sin(monthCount * Mathf.PI/180)),i*height*2.5f,-(distance * Mathf.Cos(monthCount * Mathf.PI/180)));
            coordinates[i+2] = new Vector3(0,i*height*2.5f,0);
            monthCount += 30;
        }
        /*
